...
class RBM:
    """
    Randomly adjust image brightness using a power-law transformation.

    This class applies a non-linear brightness adjustment to images, with different
    transformations for dark and bright regions based on the mean brightness.

    Methods:
        __call__: Apply brightness adjustment to input labels.
    """

    def __init__(self, p=0.5):
        """
        Initialize the RandomBrightnessAdjust object.

        Args:
            p (float): Probability of applying the transform. Must be between 0 and 1.
        """
        self.p = p

    def __call__(self, labels):
        """
        Apply brightness adjustment to input labels with probability p.
        ...
        Returns:
            (dict): The input labels dictionary with potentially adjusted image.
        """
        ...
        labels["img"] = adjust_brightness_simple(img)
        # print("augment was applied!\n")
        return labels

def adjust_brightness_simple(img):
    """
    Adjust image brightness using a power-law transformation.
    ...
    mean = np.mean(v) / 255.0
    v = v / 255
    # Interval range
    gap = mean / 2
    mask_less = v < (mean - gap)
    # Middle range can be left unchanged
    mask_more = v >= (mean + gap)

    v[mask_less] = np.power(v[mask_less], 2) / mean
    v[mask_more] = np.power(v[mask_more] - 1.0, 2) / (mean - 1.0) + 1
